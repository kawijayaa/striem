...
`)
	result, err := service.Import(context.Background(), input, false, Mapping{
		Name: "fixture", Source: "sysmon", TimestampPath: "ts",
...
func TestImportGzipJSONArray(t *testing.T) {
	...
	result, err := service.Import(context.Background(), &compressed, true, Mapping{
		Name: "compressed", Source: "test", TimestampPath: "ts", FieldPaths: map[string]string{"Message": "message"},
	})
	if err != nil {
		t.Fatalf("Import() error = %v", err)
	}
...
func TestImportNormalizesEmbeddedJSONAndCatalogsFields(t *testing.T) {
	...
	if _, err := service.Import(context.Background(), input, false, Mapping{Name: "nested", Source: "test", TimestampPath: "ts"}); err != nil {
		t.Fatal(err)
	}
	var clientIP string
...
`), false, Mapping{Name: "bad", Source: "test", TimestampPath: "ts"})
...
func TestImportCSVMapsFieldsAndNormalizesCells(t *testing.T) {
	...
	result, err := New(store).Import(t.Context(), input, false, Mapping{
		Name: "csv", Format: FormatCSV, SourcePath: "source", TimestampPath: "ts", TimestampFormat: "rfc3339",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := New(store).Import(t.Context(), strings.NewReader(test.input), false, Mapping{
				Name: "bad", Format: test.format, Source: "test", TimestampPath: "ts",
			})
